...
def remove_outliers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    log_file_preprocessing.write(f"Removendo outliers baseados na coluna {TARGET}:\n")
    log_file_preprocessing.write(f"   --> Limite inferior ({OUTLIERS_MIN_QUANTILE * 100}% quantil): {q_low}\n")
    log_file_preprocessing.write(f"   --> Limite superior ({OUTLIERS_MAX_QUANTILE * 100}% quantil): {q_hi}\n")
    log_file_preprocessing.write(f"   --> Resultando em {df_main.shape
